...
import { setupControlApiServer } from './services/airi/control-api'
...
app.whenReady().then(async () => {
  ...
  const airiHttpServer = injeca.provide('modules:airi-http-server', {
    dependsOn: { mainWindow, settingsWindow, chatWindow, widgetsManager, spotlightWindow, mcpStdioManager, godotStageManager, pluginHost, lifecycle },
    build: async ({ dependsOn }) => setupBuiltInServer({
      lifecycle: dependsOn.lifecycle,
      servers: [
        setupControlApiServer(dependsOn),
      ],
    }),
  })
